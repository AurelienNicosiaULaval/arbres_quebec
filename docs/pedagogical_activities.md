...
Calculer effectifs, moyenne, médiane, écart-type, quantiles et taux de valeurs manquantes par espèce. Discuter les résultats groupés par placette dans la table complète. Aucun poids d'inventaire n'est livré pour estimer des moyennes provinciales. Introduire la différence entre unité observée et unité d'échantillonnage.
...
Commencer avec le diamètre et la hauteur standardisés. Les 50 érables rouges ont un âge manquant dans cette version : une analyse en cas complets sur l'âge supprimerait cette classe. Exclure `basal_area_m2` si `diameter_cm` est présent afin d'éviter une variable déterministe redondante. Interpréter charges, individus et séparation des espèces.
