...
3. La surface terrière est déterministe à partir du DHP. Elle ne doit pas être traitée comme une mesure indépendante dans une ACP avec le DHP sans discuter la dépendance déterministe non linéaire.
...
5. La validation doit être groupée par placette. Une séparation aléatoire ligne par ligne peut surestimer la performance lorsque des placettes apparaissent dans plusieurs partitions.
